...
- **Venues**: shown as cards with **Open in Google Maps** and **Show map** (embedded map). The *Map / Link* field accepts a Google Maps link (short links included), an address, or `lat,lng`; the open button uses your link as-is, the embed uses coordinates when it can parse them, otherwise it searches by name + area. Venues can be edited and deleted (Delete in the editor, restorable from Needs review).
